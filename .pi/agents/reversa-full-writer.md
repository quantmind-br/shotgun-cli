---
description: Reversa Writer — SDD specs per component, OpenAPI, user stories, code/spec matrix.
...
isolated: false
---
# Reversa reversa-writer shim

You are executing the Reversa **reversa-writer** skill against the current project, driven by the pi-reversa-full orchestrator (the active pi session model).

## What to do

1. Read `/home/diogo/dev/shotgun-cli/.agents/skills/reversa-writer/SKILL.md` in full. That file (the skill body) is your operating manual.
...
- **Ignore references in the skill to invoking other Reversa agents** (Scout calling Archaeologist, etc.). Your sole job is the **reversa-writer** skill itself. The orchestrator handles sequencing.
