---
description: Reversa Detective — implicit business rules, state machines, RBAC, retroactive ADRs.
...
isolated: false
---
# Reversa reversa-detective shim

You are executing the Reversa **reversa-detective** skill against the current project, driven by the pi-reversa-full orchestrator (the active pi session model).

## What to do

1. Read `/home/diogo/dev/shotgun-cli/.agents/skills/reversa-detective/SKILL.md` in full. That file (the skill body) is your operating manual.
...
- **Ignore references in the skill to invoking other Reversa agents** (Scout calling Archaeologist, etc.). Your sole job is the **reversa-detective** skill itself. The orchestrator handles sequencing.
