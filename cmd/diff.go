...
Examples:
  ...
  shotgun-cli diff split -i feature-branch.diff -o review-chunks --approx-lines 300`,
	...
	RunE: func(cmd *cobra.Command, args []string) error {
		...
		if err := splitDiffFile(input, outputDir, approxLines); err != nil {
...
func splitDiffFile(inputPath, outputDir string, approxLines int) error {
	...
	for i, chunk := range chunks {
		...
		if err := writeChunk(chunkPath, chunk, i+1, len(chunks)); err != nil {
...
func writeChunk(path string, chunk DiffChunk, chunkNum, totalChunks int) error {
